...
@Controller
@RequestMapping(value="/usuarioGrupoCards")
public class ListarUsuarioGrupoCardsController {
	...
	public ModelAndView list(){		
		ModelAndView mav = new ModelAndView("/user/listarUsuarioGrupoCards");		
		return mav;
	}
	...
	public List<Object[]> listarGruposSemUsuariosj(@RequestParam("nome") String nome, @RequestParam("login") String login, @RequestParam("grupo") String grupo)  {
		...
		return userService.listarUsuarioGrupoCards(nome, login, grupo);
	}
	...
	private String gerarArquivoExportacaoXls(@RequestParam("nome") String nome, @RequestParam("login") String login, @RequestParam("grupo") String grupo) throws Exception {
		...
		String nomeArquivo = "usuarioGrupoCards.xls";
		
		File arquivo = new File(Constantes.caminhoArquivosXLS + File.separator + nomeArquivo);
		
		String url = arquivo.getAbsolutePath(); 
		List<Object[]> lista = userService.listarUsuarioGrupoCards(nome, login, grupo);

		String[] colunas = new String[]{"ID_USUARIO", "NM_USUARIO", "NM_LOGIN", "NM_LOGIN_UPPER", "ID_GRUPO", "NM_GRUPO"};
